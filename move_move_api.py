        ...
        while self.running and self.loco is not None:
            try:
                ...
                if self.rc_publisher is not None:
                    ...
                    # 左摇杆：控制前进/后退 (ly) 和侧移 (lx)
                    # ly > 0 = 前进, ly < 0 = 后退
                    # lx > 0 = 右侧移, lx < 0 = 左侧移
                    # 注意：摇杆值范围是 -1.0 到 1.0
                    
                    # 将速度映射到摇杆值（假设最大速度对应摇杆推满）
                    max_vx = 2.0  # 假设最大前进速度 2.0 m/s 对应 ly=1.0
                    max_wz = 1.5  # 假设最大转向速度 1.5 rad/s 对应 rx=1.0
                    ...
                    if now - last_debug > 2.0:
                        print(f"[DEBUG 遥控器模拟] ly={ly:.2f}(前进), rx={rx:.2f}(转向) | "

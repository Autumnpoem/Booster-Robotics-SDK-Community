...
CONFIG = {
    ...
    # 速度参数
    "SPEED_FORWARD": 1.8,     # 前进速度
    "SPEED_SIDEWAYS": 1.2,    # 侧向移动速度
    "SPEED_BACKWARD": -1.8,   # 后退速度

    # 圆弧行走参数
    # 圆半径 ≈ 前进速度 / 转向速度
    # 注意：速度太大可能被机器人限制！SDK示例用的是0.8/0.2    
    #建议速度1.2 角速度0.8  或者1.8和0.4
    "CIRCLE_FORWARD_SPEED": 1.8,   # 走圈时的前进速度 (m/s) - 先用小值测试
    ...
    # 直走参数
    "STRAIGHT_SPEED": 1.8,    # 直走速度 (m/s)
    
    # 预热直走参数 - 设为0禁用预热
    "WARMUP_STRAIGHT_TIME": 0,  # 预热直走时间（秒）- 0表示禁用
...
class ControllerState:
    ...
    def __init__(self):
        ...
        self.toggle_cooldown = 0.2  # 防抖时间（秒）- 缩短响应更快
        ...
        while self.running and self.loco is not None:
            try:
                ...
                if self.rc_publisher is not None:
                    ...
                    # 根据真实遥控器数据:
                    # ly 负值 = 前进 (ly=-0.98 是推满前进)
                    # rx 负值 = 左转 (rx=-0.97 是左转)
                    
                    # 将速度映射到摇杆值（摇杆范围 -1.0 到 1.0）
                    max_vx = 2.0  # 假设最大前进速度
                    max_wz = 1.5  # 假设最大转向速度
                    
                    # 左摇杆 ly = 前进速度（负值=前进，正值=后退）
                    ly = -min(1.0, max(-1.0, self.current_vx / max_vx))
                    # 左摇杆 lx = 侧移速度
                    lx = min(1.0, max(-1.0, self.current_vy / max_vx))
                    
                    # 右摇杆 rx = 转向（负值=左转，正值=右转）
                    # wz > 0 向左转，对应 rx < 0
                    rx = -min(1.0, max(-1.0, self.current_wz / max_wz))
                    ...
                    if now - last_debug > 2.0:
                        print(f"[DEBUG 遥控器模拟] event=1536, ly={ly:.2f}(前进), rx={rx:.2f}(转向) | "
                              f"原始: vx={self.current_vx:.2f}, wz={self.current_wz:.2f}")
                        last_debug = now
                else:
                    ...
                    now = time.time()
                    if now - last_debug > 2.0:
                        print(f"[DEBUG Move] vx={self.current_vx:.2f}, vy={self.current_vy:.2f}, wz={self.current_wz:.2f}")
                        last_debug = now
